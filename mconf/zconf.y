...
emptyline: T_EMPTYLINE T_EOL
{
	menu_add_entry(NULL);
	menu_add_prompt(P_EMPTYLINE, "", NULL);
	printd(DEBUG_PARSE, "%s:%d:emptyline\n", zconf_curname(), zconf_lineno());
};

comment_stmt: comment depends_list { menu_end_entry(); }
		| emptyline depends_list { menu_end_entry(); }
		;
